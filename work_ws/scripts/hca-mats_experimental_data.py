...
parser=argparse.ArgumentParser(description='Insert starting and ending date of the experimental period wanted in form %Y-%m-%d %H:%M:%S default 2021-07-15, or avg_opt= True to obtain ward for averaged data')
...
dffile = r'C:\Users\aamad\phd_scripts\codice\slides\work_ws\COVE flussi_pedonali 18-27 luglio.xlsx' #marzo 2022 Albi#
...
if args.avg_opt==True:
  if not os.path.exists(r'C:\Users\aamad\phd_scripts\codice\slides\work_ws\output\real_data\averaged'):
    os.mkdir(r'C:\Users\aamad\phd_scripts\codice\slides\work_ws\output\real_data\averaged')
  saving_dir =r'C:\Users\aamad\phd_scripts\codice\slides\work_ws\output\real_data\averaged'
else:
...
#CONVERSION TO THE FORMAT OF SIMULATION
...
if use_smooth:
  ...
  dfc = df
#DECIDO SE CONSIDERARE LA MEDIA
if args.avg_opt==False:
  ...
  for day, dfg in group:
    ...
    for j in range(0,tagn-1):
      for k in range(j+1,tagn):
        ...
        t_mat.append([distance,j,k,day.strftime('%d-%m-%Y')])
        '''      
...
#    print('shape of the distance matrix',np.shape(dist_mat))
    dfdevs = pd.DataFrame()
    ...
    dfdevs['day'] = [tags[int(a)] for a in dfdevs['tag2'].values]
    ...
    dfdevs.to_csv(os.path.join(saving_dir,'df_barriers_dist_{}.csv'.format(day.strftime('%d-%m-%Y'))))
...
#  print('mask and index',df_avg.index)
  week_mask = [True if h.weekday()>4 else False for h in dfc.datetime]
  dfc_temp=dfc.loc[week_mask]
  for e in h_mask:
  ...
  #%% DISTANCE MATRIX
  dist_mat = [] 
#  print('dataframe averaged by hour',df_avg)
  list_varco=list(df_avg.columns.drop('datetime'))
  t_mat = []
  for j in range(0,tagn-1):
    for k in range(j+1,tagn):
      if normalize:
        distance = dist(norm(df_avg[tags[j]].values[:24]),norm(df_avg[tags[k]].values[:24]),dist_type)
      else:
        distance = dist(df_avg[tags[j]].values[:24],df_avg[tags[k]].values[:24],dist_type)
...
#  print('shape of the distance matrix',np.shape(dist_mat))
  ...
  dfdevs.to_csv(os.path.join(saving_dir,'df_barriers_dist_weekend_avg.csv'))
  is_weekend=True
...
mappa = folium.Map(location=center_coords, tiles='cartodbpositron', control_scale=True, zoom_start=12)
...
if is_weekend:
  ...
  week_period = 'week'
if args.avg_opt==True:
  mappa.save(os.path.join(saving_dir,f'barriers_top_{topn}_{week_period}.html'))
else:
...
if is_weekend:
  ...
  week_period = 'week'
if args.avg_opt==True:
  mappa.save(os.path.join(saving_dir,f'barriers_worst_{topn}_{week_period}.html'))
else:
...
if is_weekend:
  ...
  week_period = 'week'
if args.avg_opt==True:
  mappa.save(os.path.join(saving_dir,'barriers_top_allverse_{}.html'.format(week_period)))
else:
...
if is_weekend:
  ...
  week_period = 'week'
if args.avg_opt==True:
  df_best.to_csv(os.path.join(saving_dir,'df_barriers_tot_{}.csv'.format(week_period)))
else:
...
if is_weekend:
  ...
  week_period = 'week'
if args.avg_opt==True:
  mappa.save('barriers_worst_singles_10_{}.html'.format(week_period))
else:
...
if is_weekend:
  ...
  week_period = 'week'
if args.avg_opt==True:
  plt.savefig(os.path.join(saving_dir,f'HCA dendrogram method_{hca_method}_distance_{dist_type}_{week_period}'))
else:
...
m = folium.Map(location=center_coords,tiles='cartodbpositron', control_scale=True,zoom_start=12)
...
if is_weekend:
  ...
  week_period = 'week'
if args.avg_opt==True:
  m.save(os.path.join(saving_dir,'barriers_all_{}.html'.format(week_period)))
else:
...
mappa = folium.Map(location=p_latlon,tiles='cartodbpositron',control_scale=True,zoom_start=12)
...
if is_weekend:
  ...
  week_period = 'week'
if args.avg_opt==True:
  mappa.save(os.path.join(saving_dir,f'barriers_locality_{barrierp}_{week_period}.html'))
else:
...
#  print('shape of the distance matrix',np.shape(dist_mat))
  ...
  dfdevs['barrier2'] = [tags[int(a)] for a in dfdevs['tag2'].values]
  dfdevs['std'] = np.std(dist_mat,axis=0)[:,0]
  dfdevs['sum'] = np.sum(dist_mat,axis=0)[:,0]# in the case of no multiple days I have just one value. So the sum is the identyty

    # add all columns before sorting
  dfstds = dfdevs.sort_values(by='std')
  dfsums = dfdevs.sort_values(by='sum')

  dfdevs.to_csv(os.path.join(saving_dir,'df_barriers_dist_week_avg.csv'))
  is_weekend=False
